...
# data recording
    record = UpdatedRecordDataJSON(
        data_prefix='Direction', data_path=trial_setup["data_path"],
        state=state, action=action, reward=reward, save_all=True)

    # environment and recording
    env = asterisk_env.AsteriskEnv(hand=hand, obj=obj)

    # sim manager
    manager = SimManagerDefault(num_episodes=len(trial_setup["goal_locations"]["y"]), env=env)

    # close_hand = close_hand_phase.CloseHand(hand, obj=obj)
    manipulation = manipulation_phase.AstriskManipulation(
        hand, obj=obj, x_goals=trial_setup["goal_locations"]["x"],
        y_goals=trial_setup["goal_locations"]["y"],
